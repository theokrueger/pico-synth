...
float get_level(Wave *wv, float newhz) {
    newhz *= wv->mult;
    float effective_time = (newhz * (wv->time - wv->start) / 1000000);
    effective_time -= (int) effective_time;
    switch (wv->type) {
        case SineWave:
            return 1 - cosf(2 * M_PI * effective_time);
        case SquareWave:
            return effective_time < 0.5;
        case SawWave:
...
//     level = level <= LVL_MIN ? 0 : (level >= LVL_MAX ? LVL_MAX : level);
    if ((level / CHANNELS) > 1) SET_LED_ON();
    else SET_LED_OFF();
    pwm_set_chan_level(wv->loc->slice_num, PWM_CHAN_A, (int) (level / CHANNELS * LVL_MAX));
